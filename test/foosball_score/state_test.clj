...
(deftest win-by-two-test
  (let [data {:scores {:black 0 :gold 0}
    ...
    (testing "Game is over for 6v4"
      (let [data (merge data {:scores {:black 6 :gold 4}})]
        (is (state/game-over? data)))
      (let [data (merge data {:scores {:black 4 :gold 6}})]
        (is (state/game-over? data))))

    (testing "Game is not over for 9v8"
      (let [data (merge data {:scores {:black 9 :gold 8}})]
        (is (not (state/game-over? data))))
      (let [data (merge data {:scores {:black 8 :gold 9}})]
        (is (not (state/game-over? data)))))))
